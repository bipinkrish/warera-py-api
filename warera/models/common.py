...
class CursorPage(WareraModel, Generic[T]):
    ...
    def from_raw(cls, raw: Any, item_type: type[T]) -> CursorPage[T]:
        ...
        if isinstance(raw, list):
            items = [item_type.model_validate(r) if isinstance(r, dict) else r for r in raw]
            return cls(items=items, next_cursor=None, has_more=False)  # type: ignore[arg-type]

        if isinstance(raw, dict):
            raw_items = raw.get("items", raw.get("data", []))
            items = [
                item_type.model_validate(r) if isinstance(r, dict) else r
                for r in raw_items
            ]
            next_cursor = raw.get("nextCursor") or raw.get("next_cursor")
            has_more = raw.get("hasMore", raw.get("has_more", bool(next_cursor)))
            return cls(items=items, next_cursor=next_cursor, has_more=has_more)  # type: ignore[arg-type]

        return cls(items=[], next_cursor=None, has_more=False)  # type: ignore[arg-type]
